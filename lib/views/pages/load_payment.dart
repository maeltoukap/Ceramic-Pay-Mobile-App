...
class LoadPay extends StatefulWidget {
  const LoadPay({Key key}) : super(key: key);

  @override
  _LoadPayState createState() => _LoadPayState();
}

class _LoadPayState extends State<LoadPay> {
  var id;
  var coupons;
  var valider;
  var date;
  ...
  getUserData() async {
    ...
    var data = await Api.listLoad(id);
    if (data != null) {
      listScanModel.clear();
      for (Map i in data) {
        setState(() {
